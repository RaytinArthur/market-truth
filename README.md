...
Market Truth 是一个处于 Week 3 阶段的金融分析智能体。核心目标是利用 GraphRAG 技术与 **Agentic Workflow**，针对特定的股价异动点，从海量新闻和市场数据中自主检索、推理，并输出高置信度的归因分析

## 🚀 Week 3 核心升级：LangGraph 工业级图流转架构与流式引擎

系统已从 Week 2 的静态“拼装式 RAG”彻底升级为**带有状态记忆、自主决策和物理硬刹车的 LangGraph 智能体引擎**，并全面接入 **FastAPI异步SSE流式输出**。大模型从“被动总结器”进化为“主动探勘者”。
...
* **Native Tool Calling **：抛弃正则解析，将双路检索封装为 OpenAI Tool Schema，由模型自主决定调用 Vector 还是 Graph。
* **O(1) 级上下文裁剪 (Memory Assassin)**：通过 `Safety Node` 和 LangGraph 的 `RemoveMessage` 机制，提炼核心 Evidence 并斩断千字以上的冗余 ToolMessage，防止 Token 雪崩和 LLM 失忆。
* **逻辑早停与物理硬刹车 (Early Stop)**：
    * **逻辑早停**：当 `evidence_pool` 集齐 `Fact (客观事件)` + `Explanation (传导逻辑)` 双要素，立即交卷，拒绝无效检索。
    * **物理刹车**：`step_count >= 5` 强行斩断循环，控制成本与响应时间。
* **异常兜底**：捕获 API 异常、图数据库断连或非法实体遍历，伪装成合法的 `ToolMessage` 引导模型优雅 Retry，保证 Call Chain 不断裂。
* **异步 SSE 流式引擎 (Telemetry)**：基于 FastAPI 与 `astream_events` (v2)，剥离模型思考流与最终报告 Token，加入心跳保活与断连监控。
...
2. **图谱规模限制**：图谱基于新闻动态构建，静态产业链全局关系网暂未接入，易触发 Fallback 降级。
